...
COURSE.addModule({
    ...
    /* ---------------- 2.1 Magyar ---------------- */
    {
          ...
          blocks: [
            {
              ...
              rows: [
                ...
                ['<code>MM</code>', 'dupla Műanyag réteg (érszigetelés + külső réteg = köpeny)', 'MM-fal / MMCu = LAPOS, fehér fal-<strong>kábel</strong> (van köpenye!)'],
            ...
            { type: 'h3', text: 'Rögzítsd a táblázatot — kösd össze a betűt a jelentésével!' },
            {
              type: 'match', gate: true, leftLabel: 'Betű', rightLabel: 'Jelentés',
              pairs: [
                { left: 'M', right: 'Műanyag (PVC) szigetelés' },
                { left: 'B', right: 'Burkolt = van köpeny (→ kábel)' },
                { left: 'KH', right: 'Különösen Hajlékony ér' },
                { left: 'G', right: 'Gumi szigetelés' },
                { left: 'MM', right: 'dupla műanyag réteg (lapos falkábel)' }
              ]
            }
          ]
        },
        {
          ...
          blocks: [
            { type: 'p', text: 'Most fejtsük meg a leggyakoribb szerelvénykábel magyar nevét. Kattints sorban a betűkre!' },
            {
              ...
              result: '<code>MBCu</code> = KÖR keresztmetszetű, <strong>világosszürke (RAL 7035)</strong> szerelvénykábel = <code>NYM-J</code>. A boltok így írják: „MBCu (NYM-J)”.'
            },
            { type: 'h3', text: 'Még két jelölés — fejtsd meg ezeket is!' },
            {
              type: 'decoder', gate: true, code: 'MCu', title: 'Fejtsd meg: ',
              parts: [
                { glyph: 'M', label: 'Műanyag', html: 'PVC érszigetelés.' },
                { glyph: 'Cu', label: 'Réz', html: 'a vezető réz; <strong>nincs B</strong> → nincs köpeny!' }
              ],
              result: '<code>MCu</code> = tömör rézvezeték köpeny NÉLKÜL = <code>H07V-U</code> — vezeték, csak védőcsőben falba.'
            },
            {
              type: 'decoder', gate: true, code: 'MKH', title: 'Fejtsd meg: ',
              parts: [
                { glyph: 'M', label: 'Műanyag', html: 'PVC szigetelés.' },
                { glyph: 'KH', label: 'Különösen Hajlékony', html: 'sok elemi szálból álló, finomsodrott ér.' }
              ],
              result: '<code>MKH</code> = finomsodrott (hajlékony) rézvezeték = <code>H07V-K</code>.'
            }
          ]
        },
        {
          ...
          blocks: [
            { type: 'h', text: 'További megfejtett példák' },
            {
              type: 'list', items: [
                ...
                '<code>MM-fal (MMCu)</code> = dupla Műanyag réteg + falba → LAPOS, fehér fal-kábel. Megfelelői a külföldi <code>YDYt</code> / <code>YMSteg</code> / <code>NYIFY</code> típusjelek (lengyel/német lapos installációs kábel — ugyanaz a termékfajta más jelöléssel)'
              ]
            },
            {
              type: 'callout', variant: 'warn', title: 'Gyakori tévesztés!',
              html: 'Az <strong>MM-fal a LAPOS</strong>, az <strong>MBCu a KÖR</strong> keresztmetszetű! Vakolat alá <strong>védőcső nélkül</strong> kifejezetten az MM-falat tervezték; az MBCu inkább falon kívül / kábelcsatornában megy. Nem ugyanaz a termék.'
            },
            { type: 'h3', text: 'Párosítsd a magyar jelet a jelentésével!' },
            {
              type: 'match', gate: true, leftLabel: 'Magyar jel', rightLabel: 'Mit jelent?',
              pairs: [
                { left: 'MBCu', right: 'köpenyes, KÖR, világosszürke = NYM-J' },
      ...
      quiz: [
        ...
        },
        {
          ...
          explain: 'MM = dupla műanyag réteg (a külső réteg a köpeny → kábel), „fal” = falba helyezhető → LAPOS, fehér. Az MBCu ezzel szemben KÖR és világosszürke.'
        },
        {
      ...
      quiz: [
        ...
        },
        {
          ...
          options: [
            ...
            { t: 'MT = H05V-K' }, { t: 'MT = H07RN-F' }, { t: 'MT = H07V-U' }
          ],
          explain: 'MT (tömlővezeték) = H05VV-F. A H05V-K finomsodrott jelvezeték, a H07RN-F a GT gumikábel, a H07V-U az MCu — mind más termék.'
        },
        {
  ...
  quiz: [
    ...
    { type: 'multi', q: 'Mely megfeleltetések helyesek? (több is)', options: [{ t: 'MCu = H07V-U', correct: true }, { t: 'GT = H07RN-F', correct: true }, { t: 'MBCu = NYM-J', correct: true }, { t: 'MT = H07V-K' }], explain: 'Az MT = H05VV-F (nem H07V-K, az az MKH). A többi három helyes.' }
  ]
});
